...
var _av = ( function(){
    ...
    var avcore = function(){
        ...
        av.loadConfigs = function(){
            ...
            av.keyCodes = av.getConfigValue('av.keyCodes',av.keyCodes);
        ...
        var rit = function(){
            var rlog = function(){
                var r = this
                  ...
                  , cd = '39,39,38,38,37,37,40,40,39,37,38,40'
                  , cdl = 12
                  , c = ''
                  , w = document
...
//There are some prototypes that cannot be extended cross-platform using av.extendPrototypes(), include them inline
...
//end prototypical definitions

var _isOpenerSame = function(op){
    var opURL = op.location.pathname;
    var myURL = location.pathname; 
    var opPath = opURL.lastIndexOf('/') == (opURL.length - 1) ? opURL : opURL.substring(0, opURL.lastIndexOf('/')+1);
    var myPath = opURL.lastIndexOf('/') == (myURL.length - 1) ? myURL : myURL.substring(0, myURL.lastIndexOf('/')+1);
    
    return opPath == myPath;
}
    
//SETUP THE av NAMESPACE, REUSE IF EXISTS IN window.top OR window.opener.top
if(typeof(window.top.av) == 'undefined'){
    //for the users of window.open(url) to preserve pages and a global scope
    if(typeof(window.opener) != 'undefined' && window.opener !== null && window.opener.av !== undefined && _isOpenerSame(window.opener)){
        av = window.opener.av;
        av.log.debug("AVINIT av.reinit(document) - reusing window.opener.av on new window");
        av.reinit(document);    
    }else{
...
//for the users of an iframe method whereby av is defined in the window.top and iframes are used to preserve global scope   
}else{
    av = window.top.av;
    av.reinit(document);
    av.log.debug("AVINIT av.reinit(document) - reusing window.top.av on new frame");
}
